...
% [result_TP_FP_FN] = leaf_evaluation_bites(bite_sign, bite_original, 5, 12, 0.5);
%
% Gabriel da Silva Vieira (INF/UFG, IFGoiano (BRAZIL) - 2022) 
%

function [result_TP_FP_FN] = leaf_evaluation_bites(...
    bite_sign, bite_original, remove_small_bites, dilate_bite, IoU_thresh)
...
[~,~,c] = size(bite_original);
if c >= 3
    gt = bite_original(:,:,2) > 0;
else
    gt = bite_original;
end
...
for i=1:length(st_bite)
    ...
    for j=1:length(st_gt)
        ...
        IoU = max(IoU_1, IoU_2);
        %IoU = sum(sum(mask_gt & mask_bite)) / sum(sum(mask_gt | mask_bite));
